...
module.exports = function(db) {
    ...
    router.post('/library/from-url', asyncHandler(async (req, res) => {
    ...
    router.post('/library/batch-delete', asyncHandler(async (req, res) => {
        const { ids } = req.body;
        if (!Array.isArray(ids) || ids.length === 0) {
    ...
    router.post('/clear-from-bookmarks', asyncHandler(async (req, res) => {
        const { iconData } = req.body;
        if (!iconData) {
            throw new AppError('缺少图标数据', 400);
        }
    ...
    router.post('/batch-clear-from-bookmarks', asyncHandler(async (req, res) => {
        const { iconDataList } = req.body;
        if (!Array.isArray(iconDataList) || iconDataList.length === 0) {
