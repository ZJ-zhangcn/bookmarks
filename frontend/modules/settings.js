...
import { closeBookmarkSearch } from './search.js';
...
function resolveSearchVisibility(config = {}, key) {
...
export async function loadPersonalization(options = {}) {
    try {
        ...
        if (config) {
            ...
            if (DOM.searchBarShow) DOM.searchBarShow.checked = resolveSearchVisibility(config, 'searchBarShow');
            if (DOM.bookmarkFilterShow) DOM.bookmarkFilterShow.checked = resolveSearchVisibility(config, 'bookmarkFilterShow');
...
export async function applyPersonalization(config, options = {}) {
    ...
    if (searchForm) {
        searchForm.style.display = resolveSearchVisibility(config, 'searchBarShow') ? '' : 'none';
    }

    if (DOM.searchContainer) {
        DOM.searchContainer.style.display = resolveSearchVisibility(config, 'bookmarkFilterShow') ? '' : 'none';
    }
